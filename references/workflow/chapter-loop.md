...
写前遇到时间压力（倒计时、截止时刻、窗口期或关键行动依赖耗时）、并行汇合（含明确要求多人
共同或同时执行）或物件交接后仍需依赖归属时，必须加载并执行专项全文。未触发时不加载；
正文证据、因果、授权与提交约束仍然适用。触发后无法读取则停止；写中或修后首次触发也须先补读再继续。

独立 Agent 必须实际收到完整章意图、required 来源内容、场景渲染规则与瞬时调用信封 `repair_round`，
不能只传链接；已触发专项也须传入实际全文。`repair_round` 不属于 Chapter Brief，也不新增持久 schema：初次调用为 0；每次提交后因 P0
...
恢复闭环仍按下文执行。
对当前候选稿做私有场景交付审计：正文证据逐项核对 `on_page_requirements` 的可定位、自然、
无歧义落地；转折桥核对改变前立场或阻力、可见触发、具体行动变化与即时后果或保留。
已触发 Step 3A 条件时，按 `references/craft/scene-action-audit.md` 全文检查动作预算、时间行动闭环与
物件归属；再次依赖该物件前，持有者或位置必须唯一可判。若草稿或修订首次触发条件，先加载全文再审计。
只在授权事实内作最小修复；不能新造历史、动机、秘密、设备或转手，不新增持久 schema、审计卡或修复轮次。
