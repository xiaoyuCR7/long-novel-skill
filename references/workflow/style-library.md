...
风格库提供候选，不是作者契约。作者认可的本书声线、禁令、叙事距离与内心戏高于来源书统计。
`apply` 会覆盖目标的 `设定/文风锚.md`，`extract --output` 也会覆盖指定文件；因此应用只指向
stage 与正式工程之外的临时候选目录，提取只输出独立指纹，再人工合并认可指标与必要节选。
不能覆盖既有样段出处、受保护特征、明确禁令或人物声线；不在章事务 stage 内修改设定。
...
| 系列写作 | 同一系列（如三部曲）需要一致的叙事腔调 | 第一部定调后导入，后续书从候选中人工选用 |
| 风格迁移 | 想换一种腔调写新书，参考某本对标书的文风 | 导入风格库后 apply 到工程外候选目录，再人工合并 |
| 风格实验 | 同时尝试几种不同腔调，对比哪种更适合新书 | 分别导入多种风格，apply 到不同草稿，对比效果 |
| 团队协作 | 多人合写需要统一文风基准 | 主笔创建风格条目，协作者读取认可契约，不对正式章节 apply |
...
源书必须已有含实测指标的 `设定/文风锚.md`（手工维护契约）或 `设定/文风指纹.md`。
如果没有，先对源书样章提取：

```bash
python scripts/style_fingerprint.py extract "正文/第001章.md" "正文/第002章.md" \
  --output "{工程外工作目录}/源书指纹.md"
```

将认可的指标人工合并到源书锚，或另存非契约的 `设定/文风指纹.md`；已有同名文件先核对。
导入器优先读取文风锚，所以锚已存在时不能只更新指纹就假定导入器会采用新数值。
...
4. 生成唯一 ID，写入 `{skill_root}/assets/style_library/index.json`
...
2. 生成候选目录下的 `设定/文风锚.md`（量化基线、高频词、腔调关键词、锚点片段）
3. 如果目标目录的 `设定/` 不存在，自动创建

### Step 3：手工校准

比较候选与正式锚，仅人工合并作者认可的指标/节选，不把整个候选覆盖正式锚。
正式锚用 `assets/templates/style-anchor.md`；未提供样段不阻断其他已具备条件的工作。
应用后需人工确认的内容（脚本不会自动填写）：

| 待确认项 | 位置 | 操作 |
|---|---|---|
| 腔调关键词 | 设定/文风锚.md > 腔调关键词 | 按作者要求定义，不凑数量，不直接照搬风格标签 |
| 不用的腔调 | 设定/文风锚.md > 腔调关键词 | 明确新书不会用的腔调 |
| 高频词白名单 | 设定/文风锚.md > 高频词白名单 | 新书合理的高频词 |
| 样板段落 | 设定/文风锚.md > 样板段落 | 注明来源与作者认可依据，有认可新样段时人工更新，未提供可留待补 |
| 手工契约 | 设定/文风锚.md > 作者依据与受保护特征 | 保留明确禁令、叙事距离、内心戏及叙述者/人物区别 |
...
```bash
python scripts/style_fingerprint.py extract "正文/第001章.md" "正文/第002章.md" \
  --output "{工程外工作目录}/新书指纹.md"
```

这只生成独立指纹。核对样章是否获作者认可及样本范围，再人工更新正式锚的相关指标，
不覆盖手工契约，不因数值偏离来源书就强改正文。
...
| 备份索引 | 每月 | 备份 `{skill_root}/assets/style_library/index.json` 和 `snippets/` 目录 |

### 版本管理

风格库的索引文件 `{skill_root}/assets/style_library/index.json` 是纯 JSON，适合用 Git 管理：
...
| 文件 | 来源 | 用途 |
|---|---|---|
| `设定/文风锚.md` 或 `设定/文风指纹.md` | 人工契约合并指标 / 独立指标文件 | 风格指纹源数据；导入器优先读锚 |
...
| 文件 | 写入时机 | 内容 |
|---|---|---|
| `{工程外工作目录}/风格候选/设定/文风锚.md` | apply 时 | 待人工审阅的六维基线 + 关键词 + 节选，不是正式锚 |
...
风格库存储来源书的指标；apply 只生成候选，新书正式锚由作者要求与认可样段决定。
新样章可用 extract 输出独立指纹，再人工合并指标。来源书数据仅作参考，不覆盖手工契约。
...
可以。风格库是纯本地文件，`{skill_root}/assets/style_library/index.json` 和 `snippets/` 目录可以复制到其他机器。
