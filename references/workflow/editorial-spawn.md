...
涉及虚构机制时，各角色的实际派发上下文同时包含本章相关世界规则与
`references/craft/fictional-world.md` 的适用内容，不只给链接；现实科学或法律不是超凡剧情可行性的门禁。
...
**初始化本轮状态**：初次初始化 0 只在无同章同任务历史时写入 `追踪/editorial_state.json`；
已有同章同任务状态时不得覆盖其 `rewrite_round`，须按 Step 4 映射后继续：
...
**目的**：写作特工接收 Chapter Brief 与瞬时调用信封，输出纯正文，严格隔离 meta 信息。

**写作特工输入**（SendMessage 传递，只给 Brief 与调用信封，不给其他上下文）：

`repair_round` 是不属于 Chapter Brief、也不新增持久 schema 的瞬时调用信封。每次 Step 4 SendMessage
都从现有持久 `追踪/editorial_state.json.rewrite_round` 读取并映射为 `repair_round`：首次无值为 0；
恢复会话读取同一持久值；恢复会话沿用已有值，不得默认重置。每次提交后因 P0 重写递增，最多 2，并传入下一次调用。
...
**输出分流（先于正文检查）**：先解析写作特工的互斥输出。合法无正文 `BLOCKED` 的
`required_context_missing`、`required_context_over_budget`、`outline_underfilled`、`scene_audit_blocked`
或 `gate_blocked` 一律传播阻断并停止，不进入 NOVEL_TEXT 标记/标记外污染检查，不自动重写、不额外消耗轮次。
格式非法的输出才按 P0/阻断报告；不得把合法 BLOCKED 吞掉或误判为正文污染。只有成功分支才进入下列正文隔离检查。

**正文隔离检查**（P0 触发器，成功分支的写作特工输出后立即检查）：
- 标记外有多余内容 → P0，重写。
- 正文含 `[说明]`/`TODO`/大纲语言 → P0，重写。
- 字数偏差 >20% → P0，重写。
- 连续 3 段以上「他/她」开头句式雷同 → P0（AI 腔信号），重写。

P0 触发 → 不进入 Step 5。先原子更新并持久化
`editorial_state.json.rewrite_round = min(previous + 1, 2)`（`previous` 读取同一持久字段），再从该值构造
下一次 `repair_round` 并发送下一次 Step 4 SendMessage；不得只递增瞬时变量。`repair_round` 达到 2 后仍有
P0 则停止重写，按 `gate_blocked` 处理。
...
核查本书规则与当前条件，不以现实不存在或违反物理/现实法律判错。已授权复生、瞬移、系统奖励可以成立；
合法测时可以来自已设定的系统、法阵或神识。每个冲突须给出正文与本书规则的双向证据。
...
完整 Chapter Brief（含 on_page_requirements）：
[CHAPTER_BRIEF]
{粘贴完整 Brief}
[/CHAPTER_BRIEF]
...
共享场景渲染/审计规则的实际内容：
[SCENE_AUDIT_RULES]
{粘贴 scene-rendering.md 的场景渲染与场景交付审计卡实际内容}
[/SCENE_AUDIT_RULES]

输出 S1-S4 核查报告：
S1：角色地理位置与状态文件一致性
S2：角色能力边界未违反
S3：死亡/离开后再次出现符合本书机制与当前条件（回忆、幻觉、复生、分身或回归等）
S4：伏笔操作与台账一致（该埋的埋了、该收的收了）
场景交付审计：核查硬锚点、转折桥、倒计时行动预算与时间行动闭环、道具交接。限时行动先核查输入、设定或
已在正文建立事实中的合法时间来源，不得新造计时器、刻度、警报或其他测时来源；再核查合法报时、
或其他非计时过程形成的时间证据是否至少一项改变后续行动，以及读者能否判断哪一步消耗窗口及为何
此刻改变选择。承担主要窗口消耗的串行或并行分支，不得只写起点和完成口令；须用授权动作中的中间进展、
阻力变化或累积身体/环境反馈，让读者能重建该分支如何耗时；不得为此新增动作、工具、事实或机械报时。
若输入明确限定倒计时或显式读数只在改变行动选择时出现，保留的每条报时都须影响后续
行动；纯进度读数删除或改用非计时过程证据。输入无此限定时，仍按整体至少一项时间证据改变行动，
不扩张为每次报时的全局要求。现场已有合法倒计时且限时行动跨越多个阶段时，若后续某个剩余时间读数会
实际改变行动范围、风险判断或验收选择，须在该决策点呈现该读数；不得以避免机械报时为由省略有功能的
后续读数。若后续没有新的选择变化，则不新增读数；仍不设最低报时次数。若输入或 `on_page_requirements`
将受保护下游动作或状态锁在多个前置
分支的汇合条件之后，并行准备仅可推进到汇合屏障；全部前置条件满足之前，任一分支的中间效果不得
提前触发或造成受保护下游动作或状态变化。提前触发属于既有顺序错误，按 blocking/P0 报告。只有输入
明确把受保护下游动作指定为多名参与者共同或同时执行时，汇合完成只解除前置屏障；全部指定参与者就位
前，任何一人不得单独启动，动作须从起始即共同或同时执行。“一人先启动、他人随后加入”仍属于既有
顺序错误，按 blocking/P0 报告。只有上述
显式报时/读数功能违例且没有独立 P0 时，才归 P1、不阻断；如还存在既有 P0 条件，P0 优先。上述显式
报时/读数 P1 的 `minimal_fix` 只处理该读数：纯进度读数删除或替换为非计时过程证据；遗漏有功能的后续
读数只在对应决策点补入来自既有合法时间来源的读数；不得改写无关动作或新增工具。其他类别按各自问题作最小修复。道具交接须在再次依赖该物件前唯一可判。仅当输入或
`on_page_requirements` 明确要求锚点在正文出现而该锚点缺失，或关键事实或整体因果无法判定、时限与
动作链冲突、未授权计时器或测时来源、关键物件归属不明，才是 blocking/P0。正文结果已可判定，
整体因果也已可判定，动作链可行；但读者仍无法重建窗口消耗，或无法判断时间证据如何改变后续选择，归 P1，应修复但不阻断；
时长与选择已可重建但压力递进或自然度可润色，归 P2，建议但不阻断。不设固定报时频率或动作配额。
每项标注 pass/fail + 具体问题。
只报告问题，不另起第二套修复轮次；P0 交回总编辑，复用当前 `rewrite_round` 的统一流程。
```

连载核实官的所有 P0 返回总编辑，并复用当前 `rewrite_round` 的统一流程。
...
- 连载核实官报告 P0：违背本书已明确复生条件/主线矛盾/作品内时间规则冲突/伏笔台账不一致；
  不因复生、瞬移或明确的时间回溯本身判错。
- 正文出现 `[说明]`/`TODO`/分析性段落（正文隔离 P0 触发器，见 `editorial-team.md`）。
- 共享场景审计的硬锚点、倒计时行动预算与道具交接中，仅当输入或 `on_page_requirements` 明确要求锚点
  在正文出现而该锚点缺失，或关键事实或整体因果无法判定、时限与动作链冲突、
  未授权计时器或测时来源、关键物件归属不明，才是 blocking/P0；
  转折桥按同一关键因果标准核查。
- 若输入或 `on_page_requirements` 将受保护下游动作或状态锁在多个前置分支的汇合条件之后，并行准备
  仅可推进到汇合屏障；全部前置条件满足之前，任一分支的中间效果不得提前触发或造成受保护下游动作
  或状态变化。提前触发属于既有顺序错误，按 blocking/P0 处理。只有输入明确把受保护下游动作指定为
  多名参与者共同或同时执行时，汇合完成只解除前置屏障；全部指定参与者就位前，任何一人不得单独启动，
  动作须从起始即共同或同时执行。“一人先启动、他人随后加入”仍属于既有顺序错误，按 blocking/P0 处理。

**P1 判定标准**：
- 反AI编辑报告 P1：节奏断裂/人设偏差/钩子缺失。
- 连载核实官报告 P1：角色位置逻辑可疑/能力边界擦边。
- 共享场景审计的 P1/P2 边界是：正文结果已可判定，整体因果也已可判定，动作链可行；但读者仍无法重建窗口消耗，
  或无法判断时间证据如何改变后续选择，归 P1，应修复但不阻断；时长与选择已可重建但压力递进或自然度可润色，归 P2，建议
  但不阻断。
- 承担主要窗口消耗的串行或并行分支，不得只写起点和完成口令；须用授权动作中的中间进展、阻力变化或
  累积身体/环境反馈，让读者能重建该分支如何耗时；不得为此新增动作、工具、事实或机械报时。
- 若输入明确限制倒计时或报时仅在改变程序选择时出现，保留的显式读数须各自改变后续选择；纯进度
  报时删去或换作非计时过程证据。输入没有该限制时，仍执行整体至少一项时间证据改变行动，不推成逐条
  全局义务。只有上述显式报时/读数功能违例且不存在独立 P0 时，才归 P1、不阻断；若同时存在既有 P0，P0 优先。
...
结束，是否恢复？」→ 恢复则从断点 Step 继续；不恢复则先 TeamDelete 后重新开始，但同章同任务的
`rewrite_round` 必须保留，不得清零。
...
返工时不换策划主编（Brief 没问题就不重新生成），只让写作特工重写。若 Brief 本身
有问题（情节点逻辑不通导致怎么写都过不了门禁），可回策划主编重新生成 Brief；但 Brief 重生成
不得让同章同任务的共享修复轮次清零，恢复或当前章节返工都不能绕过两轮上限。只有作者明确拆分为
作者明确宣布真正的新任务并在开始前记录新的任务边界，才可另起计数；换团队、会话或重做 Brief 都不得
用来替当前章节返工重置。
